...
/**
 * Formata o nome do cliente para ter apenas a primeira letra maiúscula.
 * Caso o nome venha em CAIXA ALTA, converte para "Primeira letra maiúscula, restante minúsculo".
 * @param {string} name - Nome a ser formatado
 * @returns {string} Nome formatado
 */
function formatCustomerName(name) {
  if (!name) return name;
  // Verifica se o nome está todo em maiúsculas (CAIXA ALTA)
  if (name === name.toUpperCase()) {
    return name.charAt(0).toUpperCase() + name.slice(1).toLowerCase();
  }
  // Se já tiver formatação mista, retorna como está
  return name;
}
...
function insertMessage(message) {
  ...
  if (finalMessage.includes('[NOME_CLIENTE]')) {
    ...
    if (customerFirstName) {
      const formattedName = formatCustomerName(customerFirstName);
      finalMessage = finalMessage.replace(/\[NOME_CLIENTE\]/g, formattedName);
    } else {
